from tkinter import *
import math
import tkinter.messagebox
# ---------------------------- CONSTANTS ------------------------------- #
PINK = "#e2979c"
RED = "#e7305b"
GREEN = "#9bdeac"
YELLOW = "#f7f5dd"
FONT_NAME = "Courier"
WORK_MIN = 25
SHORT_BREAK_MIN = 5
LONG_BREAK_MIN = 20
reps = 0
tick = "✅"
timer = None

# ---------------------------- TIMER RESET ------------------------------- #


def restart_timer():
    global reps
    window.after_cancel(timer)
    timer_label.config(text="Timer")
    canvas.itemconfig(timer_text, text="00:00")
    tick_label.config(text="")
    reps = 0
# ---------------------------- TIMER MECHANISM ------------------------------- #


def start_timer():
    global reps
    global tick
    reps += 1
    if reps % 8 == 0:
        timer_label.config(text="Break", fg=RED)
        count_down(20 * 60)
    elif reps % 2 == 0:
        timer_label.config(text="Break", fg=PINK)
        count_down(5 * 60)
    else:
        timer_label.config(text="Work", fg=GREEN)
        count_down(25 * 60)

    # ---------------------------- COUNTDOWN MECHANISM ------------------------------- #


def count_down(count):
    global reps
    global tick
    count_min = math.floor(count // 60)
    count_sec = count % 60
    if count_sec < 10:
        count_sec = f"0{count_sec}"
    canvas.itemconfig(timer_text, text= f"{count_min}:{count_sec}")
    if count> 0:
        global timer
        timer = window.after(1000, count_down, count-1)
    else:
        start_timer()
        if reps % 2 == 0:
            tick_label.config(text=tick, font=('bold'))
            tick += tick
        if reps != 8:
            tkinter.messagebox.showinfo("Pomodoro", "Work time is over. Take a break!")
        window.focus_force()
# ---------------------------- UI SETUP ------------------------------- #


window = Tk()
window.title("pomodoro")
window.config(padx=20, pady=10, bg=YELLOW)

timer_label = Label(text="Timer", fg =GREEN, bg=YELLOW, font=(FONT_NAME, 40, 'bold'))
timer_label.grid(row=0, column=1)

canvas = Canvas(width=200, height= 224, bg=YELLOW, highlightthickness=0)
logo_img = PhotoImage(file="tomato.png")
canvas.create_image(100, 112, image=logo_img)
timer_text = canvas.create_text(100, 138, text="00:00", fill="white", font=(FONT_NAME, 35, "bold"))
canvas.grid(row=1, column= 1)

tick_label = Label(text="", fg=GREEN, bg= YELLOW, font='bold')
tick_label.grid(row=3, column=1)

start_button = Button(text="start", font=(FONT_NAME,10, 'bold'), command=start_timer)
start_button.grid(row=2, column=0)
reset_button = Button(text="reset", font=(FONT_NAME,10, 'bold'), command=restart_timer)
reset_button.grid(row=2, column=2)

window.mainloop()
